...
| **M10 + M10b + M11a** (primary for H2 coverage-rate question) | 10 | 5 (test_v4, test_v6, test_v8, test_v11, test_v12) | 50.0% | [18.7%, 81.3%] | 3 (test_v4, test_v8, test_v12) | 30.0% | [6.7%, 65.2%] | 2/5 |
...
- M10 + M10b + M11a: **3/10** — adds test_v4 V2-Opus false/h = 7.128/h on M10's test_v4 carry-forward (auto-surf bypass via surprise-gate z<-0.5 on `designgrid_renewal` + `calendar_feature_tip`; arbiter never consulted on these events — mechanism is surprise-gate-driven, distinct from test_v12's V2-prompt YES-bias mechanism but produces the same bytewise-identical false-init events as V2-3B since both tiers share the local surprise gate). Carry-forward counting error in M11a's initial aggregation surfaced during M11b Commit D analysis; corrected herein under the strict joint-bar definition (`hit < 0.80 OR false/h > 5.0/h`, pre-reg line 224).
...
| M10 + M10b + M11a N=10 | 50% (5/10) | 3 | `any V2-3B rate AND V2-Opus > 0 AND V2-Opus ≤ V2-3B (3 ≤ 5)` | **Row 4a** |
...
> *"Across 10 fresh externally-authored traces under three protocol generations (M10 frozen list; M10b frozen list; M11a iteratively-extended list), V2-Opus failed the joint bar on 3/10 traces. V2-Opus failure count (3) ≤ V2-3B failure count (5); model-scale is load-bearing-but-not-sufficient under the strongest available external sample. Per-failure mechanism: (a) test_v8 V2-Opus hit = 0.60 < 0.80 driven by 2 V2-enumeration limits at Opus scale (mom_birthday_heads_up + bridgers_presale_window — discretionary entertainment + family-event reminders outside V2's literal 'deadline obligation' enumeration); (b) test_v12 V2-Opus false/h = 7.742 > 5.0/h driven by V2-prompt-inherited YES-bias on arbiter-consulted distractors grocer_back_in_stock + calendar_yoga_suggest that V2-3B also surfaces bytewise-identically; (c) test_v4 V2-Opus false/h = 7.128 > 5.0/h driven by surprise-gate auto-surf bypass on designgrid_renewal + calendar_feature_tip (both events have z < −0.5 relative to the local-predictor expectations so they bypass the arbiter entirely and surface directly; V2-3B exhibits bytewise-identical behavior on the same two events because both tiers share the local surprise gate — the mechanism is surprise-gate-driven, distinct from (b)'s V2-prompt YES-bias mechanism, and V4 prompt revisions do not address (c) since the arbiter is never consulted). M10b's test_v8 partial-closure pattern replicates in the M11a sample. Combined-N=10 V2-3B failure rate is 5/10 = 50.0% (95% CI [18.7%, 81.3%]) and V2-Opus failure rate is 3/10 = 30.0% (95% CI [6.7%, 65.2%]). H2 is confirmed in direction (V2-Opus reduces failure rate from 50% → 30%, a 20 pp reduction with non-overlapping point estimates but partially overlapping CIs) but not in absolute (residuals remain). Future work (M11+) addresses via routing / V4 prompt / model-family upgrade / surprise-gate retuning depending on the residual mechanism."*

**Commit-D correction note (M11b-Commit-D-surfaced; correction commit lands prior to M11b Commit D):** The integer placeholder fills in this paper-line above (`3/10 = 30.0%`, CI `[6.7%, 65.2%]`, three-mechanism breakdown with item (c)) replace the original M11a Commit D `cceeddd` fills (`2/10 = 20.0%`, CI `[2.5%, 55.6%]`, two-mechanism breakdown). The original M11a aggregation missed test_v4 V2-Opus's joint-bar failure (hit = 0.80 borderline-PASS but false/h = 7.128 > 5.0/h FAIL) when carrying M10's test_v4 cell forward into the combined-N=10 metric. The strict joint-bar definition `hit < 0.80 OR false/h > 5.0/h` (pre-reg line 224 + line 1829) is the locked artifact; integer counts are derived from it. The underlying carry-forward data is unchanged — M11b Commit B's belt-and-suspenders bit-identical re-run verified test_v4 V2-3B at M11b SHA `c562173` matches M8b's `15a-content-test_v4.json` exactly, and M10's `17b-content-opus-v2-test_v4.json` (V2-Opus hit_rate=0.8, false_initiation_rate_per_hour=7.128712871287129) is unchanged from M10 SHA `68d42e3` → M11a SHA `cceeddd` → M11b SHA `c562173`. Three distinct V2-Opus residual mechanisms across the combined-N=10 sample now mapped to three distinct next-step levers: V4 prompt expansion (M11a-extension) addresses (b); model-family-tier swap (M11b) targets (a); surprise-gate retuning or z-band tightening targets (c).
...
**Scope of the V2-prompt YES-bias finding (Commit-D correction note):** This independent finding applies to test_v11 (partial: false-init at false/h=3.711, joint-bar still PASS) and test_v12 (full: false-h=7.742, joint-bar FAIL) — two trace examples where V2-Opus and V2-3B agree bytewise on the arbiter-consulted YES decision for borderline distractor content. The correction-note-restored test_v4 V2-Opus joint-bar failure (per the Commit-D correction note in the prior subsection) is **not** an instance of this V2-prompt YES-bias mechanism: its two false-inits (`designgrid_renewal` + `calendar_feature_tip`) bypass the arbiter entirely via surprise-gate auto-surf (z<−0.5), so V2's prompt enumeration is not the load-bearing factor. The V2-prompt YES-bias mechanism evidence stands at 2 trace examples (test_v11 + test_v12), not 3 — test_v4 is a third residual mechanism (surprise-gate auto-surf bypass) discussed in the corrected paper-line above.
...
- H2 coverage-rate verdict at combined-N=10 primary scope: **Row 4a — partial-closure with residuals**. V2-3B failure rate 50% (CI [18.7%, 81.3%]); V2-Opus failure rate 30% (CI [6.7%, 65.2%]); V2-Opus closes 2 of 5 V2-3B failures on joint bar (test_v6, test_v11) and partial-closes 3 of 5 on hit-side (test_v4, test_v8, test_v12: hit-rate improvement but joint-bar still fails); 3 residual V2-Opus joint-bar failures via 3 distinct mechanisms — V2-enumeration limit at Opus scale (test_v8 hit-side), V2-prompt-inherited YES-bias on arbiter-consulted borderline distractors (test_v12 false-h-side), surprise-gate auto-surf bypass on high-surprise distractors (test_v4 false-h-side; V2-3B-identical because the surprise gate is local-deterministic). Model-scale is load-bearing-but-not-sufficient. Pareto headline 13-33× cheaper per hit than poll-Opus survives the expanded sample. *(Updated from M11a Commit D's original 2/10 = 20% per Commit-D correction note above; see correction commit for full audit trail.)*
- Independent finding: V2-prompt-inherited YES-bias on retail/calendar-suggestion/social-meetup distractors (V2-Opus inherits V2-3B's permissiveness rather than improving selectivity) holds for 2 of the 3 V2-Opus joint-bar failures (test_v11 + test_v12); future V4 prompt revisions are the more targeted lever than further model-scale upgrades for those two. The third V2-Opus joint-bar failure (test_v4) is a distinct surprise-gate-auto-surf-bypass mechanism that V4 prompt revisions do not address.
