...
**Milestone:** M8b — externally-authored held-out trace `test_v4` (runs/15) pre-registered under a tightened spec that added `all(kw.lower() in event.content.lower() for kw in keywords)` as a hard constraint, generated in a fresh Claude Code session with no access to agent internals, executed under the frozen M7 config. **Primary P1 FAIL in the `< 0.60` branch**: content arbiter scores 0.40 on `test_v4` (misses `parking_meter_oak`, `cover_standup_request`, `protest_commute_route`) while poll, random p=0.75 seed=42, and cron30s all reach 1.00 — so P4 PASSes (trace is fair) and the M8 "unfair trace" defense does not apply. All three misses tag as **V2-prompt coverage gaps** (urban warnings, colleague social asks, civil-disruption commute alerts) per `llm_stats.arbiter_yes_rate = 0.0` — the arbiter said NO to every event it was consulted on, correctly on 2 distractors and incorrectly on 3 GTs. Zero band-edge misses, zero predictor-latches. Per the pre-reg's P1 `< 0.60` decision rule, no config change on `test_v4`. **M6a's headline is falsified on an externally-authored, spec-compliant trace** — the M8b protocol delivered the "design-your-own-eval" review-attack closer honestly, in the direction that surfaces a real weakness the curated-trace validation could not. M6a/M7 claims remain valid as **three-trace-specific** claims on the co-developed traces (`dev_v2` / `test_v1` / `test_v2`); the implied four-trace extension is withdrawn.
**Next:** M9 candidate — re-architect the V2 prompt from a closed list of YES regimes (scheduling / deliveries / weather / …) to a principled criterion covering the broader GT distribution an external author naturally produces, validated single-shot on `test_v5` under the same M8b protocol (banned ids/themes/pairs extend with `test_v4`'s contributions). M7b (N=20 seed sweep on `test_v4`) is a cheap drop-in if reviewers push for seed-robustness on the external trace too, but does not change the P1 verdict.
...
- **Paper framing (M8b-updated):** *Surprise-gated selective initiation with a content arbiter (V2 prompt, co-developed on three curated traces) delivers hit ≥ 0.80 on every co-developed trace at 6.8–11.3× lower token cost per correct proaction than an unconditional poll baseline, under a single frozen configuration. Mechanism attribution confirmed via matched-firing-rate random ablation, seed-variance hardened on test_v2 across N = 20 random-arbiter seeds. **Under the M8b external-authoring protocol (fresh Claude Code session authors a spec-compliant trace with no agent context), the V2 prompt's coverage fails on urban warnings / colleague asks / civil-disruption alerts: hit drops to 0.40 on `test_v4` where poll, random, and cron all reach 1.00. All three misses tag as V2-prompt-coverage gaps, not band-edge or predictor-latch. The external-authoring protocol is the methodological contribution that surfaced this.*** This is a better paper than "hit ≥ 0.80 on every trace" — the curated-trace headline is honest about its scope, and the external-authoring protocol is demonstrated to be load-bearing rather than decorative.
