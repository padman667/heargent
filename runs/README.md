...
**Milestone:** M7 — seed-variance ablation (runs/13) pre-registered, executed, **partially validated**. S1 (seed=42 equivalence), S3 (C3 on test_v2 seed-robust at 18 / 20), and S5 (arbiter-call invariant across 60 cells) pass. S2 (C3 on test_v1 at the 5.0/h bar) fails at 15 / 20 (content still strictly better on false/h in 20 / 20; the tail just crosses the pre-reg bar). S4 records dev_v2 at 0 / 20, as predicted — content's 0.00 false/h makes Δfalse/h ≤ −5.0 structurally unreachable. Per the pre-reg decision rules, this is the "partial hardening" branch: C3 becomes trace-specific. M6a's main headline (single-config hit ≥ 0.80 everywhere at 6.8–11.3× lower tok/hit than poll) is unaffected.
**Next:** M6b (Claude-API arbiter) remains the next lever for the `package_arrival` residual. M7b (N=200 seed scale-up) is cheap drop-in code if reviewers push back on S2; unnecessary for the current paper framing (report the Δfalse/h CI, not a binary bar).
