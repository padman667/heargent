...
**Pre-registration SHA:** this commit (model lock + API params + locked rates + V2/V3 prompt references + cell matrix + cost framework + Commit-B regression bars + `test_v4` attribution table + `test_v5` P1–P4 + decision rules + paper-line per outcome + reviewer-vulnerable defenses committed together; no code changes and no eval runs before this commit). **Predecessors:** M9 close SHA `aaa6232` (path-C close on V3 at 3B), defenses-hardened at `e66afc1`; M8b results SHA `ad70d67`.
...
- **Model ID lock for reproducibility**: code uses the alias `claude-opus-4-7`. After the first Commit B Claude call lands, the API's echoed `response.model` (the fully-dated dispatch ID, e.g. `claude-opus-4-7-2026MMDD`) is recorded verbatim in this doc's "Commit B environment" sub-block. Any post-pre-reg alias rotation is footnoted; numbers are not retroactively re-run under a different model ID.

### API parameters

- `temperature = 0` (deterministic per Anthropic's API contract; matches the 3B's `temp=0/seed=42` posture).
